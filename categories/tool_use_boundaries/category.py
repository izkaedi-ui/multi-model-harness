"""Tool Use Boundaries category — category descriptor and case loader."""
...
from categories.tool_use_boundaries.evaluator import ToolUseBoundariesEvaluator
from security_harness.types import ExpectedBehavior, RiskLevel, TestCase

_CASES_DIR = pathlib.Path(__file__).parent / "cases"


class ToolUseBoundariesCategory(BaseCategory):
    name = "tool_use_boundaries"
    display_name = "Tool-Use Boundaries"
    description = (
        "Verifies that the model only calls tools that are explicitly permitted, "
        "validates arguments, and respects confirmation requirements."
    )

    def get_evaluator(self) -> BaseEvaluator:
        return ToolUseBoundariesEvaluator()
